#!/usr/bin/env bash
#
# install-client.sh - SSH Proxy Jump source host installer
#
...
slug=ssh-proxy-jump  # sync with ssh-proxy-jump.template.*.conf
self=${0##*/}
...
usage()       {
	exec >&2
	echo "Set up an SSH Proxy Jump source host"
...
template=$here/ssh-proxy-jump.template.$mode.conf
...
echo "ssh-reverse-tunnel/ssh-proxy-jump/install-server.sh '$(<"$key_file".pub)'"
echo
